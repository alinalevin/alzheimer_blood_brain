"""Brain classification pipeline converted from `Copy_of_brain_ad_classifier.ipynb`.
Provides reusable functions for AD vs Control and MCI vs Control tasks leveraging
utilities defined in `classification_eval.py`."""
...
class TaskTee:
    def __init__(self, task_name: str, log_dir: Path):
        ...
        self.log_path = log_dir / f"brain_{safe_task}_{date_str}.log"
...
def describe_dataset(tissue_name: str, x_train, y_train, x_test, y_test, train_metadata: pd.DataFrame, test_metadata: pd.DataFrame) -> None:
    """Print summary statistics about the dataset, including tissue type and diagnosis counts."""
    print(f"\n{'='*60}")
    print(f"{tissue_name} Dataset Summary")
    print(f"{'='*60}")
    print(f"Train samples: {x_train.shape[0]}")
    print(f"Test samples: {x_test.shape[0]}")
    print(f"Number of genes (features): {x_train.shape[1]}")
    print(f"\nTrain diagnosis counts:")
    print(pd.Series(y_train).value_counts())
    print(f"\nTest diagnosis counts:")
    print(pd.Series(y_test).value_counts())
    if 'tissue' in train_metadata.columns:
        print("\nTrain tissue type counts:")
        print(train_metadata['tissue'].value_counts())
    if 'tissue' in test_metadata.columns:
        print("\nTest tissue type counts:")
        print(test_metadata['tissue'].value_counts())
    print(f"{'='*60}\n")


@dataclass
class BrainClassifierConfig:
    metadata_path: Path
    counts_path: Path
    train_metadata_path: Path
    test_metadata_path: Path
    dlpfc_ad_genes_path: Path
    dlpfc_mci_genes_path: Path
    pcc_ad_genes_path: Path
    pcc_mci_genes_path: Path
    outlier_ids: List[str]
    feature_alpha: float = 0.001
    feature_l1_ratio: float = 0.05
    tissue_name: str = "Brain"
    output_dir: Optional[Path] = None

    @staticmethod
    def default(out_dir: Optional[Path] = None) -> "BrainClassifierConfig":
        data_dir = Path(__file__).resolve().parent / "files" / "brain"
        if not data_dir.exists():
            raise FileNotFoundError(f"Expected data directory at {data_dir}")
        return BrainClassifierConfig(
            metadata_path=data_dir / "brain_metadata_after_preprocess_all.xlsx",
            counts_path=data_dir / "brain_residuals_all.csv",
            train_metadata_path=data_dir / "brain_train_metadata_all.xlsx",
            test_metadata_path=data_dir / "brain_test_metadata_all.xlsx",
            dlpfc_ad_genes_path=data_dir / "DLPFC_dge_ad_control_meta_results.xlsx",
            dlpfc_mci_genes_path=data_dir / "DLPFC_dge_meta_results_MCI_AD.xlsx",
            pcc_ad_genes_path=data_dir / "PCC_dge_meta_results_AD_CONTROL.xlsx",
            pcc_mci_genes_path=data_dir / "PCC_dge_meta_results_MCI_AD.xlsx",
            outlier_ids=[],
            output_dir=out_dir,
        )
...
def load_counts(path: Path, specimen_ids: Optional[List[str]] = None) -> pd.DataFrame:
...
def _prepare_gene_frame(df: pd.DataFrame) -> pd.DataFrame:
    if df.index.name == "ensembl_gene_id":
        df = df.reset_index(drop=False)
    if "external_gene_name" in df.columns:
        df = df.rename(columns={"external_gene_name": "symbol"})
    if "ensembl_gene_id" not in df.columns:
        raise ValueError("Gene frame must contain an 'ensembl_gene_id' column")
    df = df.drop_duplicates(subset=["ensembl_gene_id"])
    df.set_index("ensembl_gene_id", inplace=True, drop=False)
    return df


def load_brain_gene_sets(config: BrainClassifierConfig) -> Tuple[pd.DataFrame, pd.DataFrame]:
    dlpfc_ad = _prepare_gene_frame(pd.read_excel(config.dlpfc_ad_genes_path))
    dlpfc_mci = _prepare_gene_frame(pd.read_excel(config.dlpfc_mci_genes_path))
    pcc_ad_raw = _prepare_gene_frame(pd.read_excel(config.pcc_ad_genes_path))
    pcc_mci_raw = _prepare_gene_frame(pd.read_excel(config.pcc_mci_genes_path))

    ad_intersection = dlpfc_ad.index.intersection(pcc_ad_raw.index)
    mci_intersection = dlpfc_mci.index.intersection(pcc_mci_raw.index)

    ad_genes = dlpfc_ad.loc[ad_intersection].copy()
    mci_genes = dlpfc_mci.loc[mci_intersection].copy()
    return ad_genes, mci_genes
...
def prepare_task_data(
    ...
    selected = feature_df["ensemble_gene_id"].tolist()
...
def train_balanced_random_forest(task_data: TaskData) -> BalancedRandomForestClassifier:
    rf_params = {
        ...
        "criterion": "entropy",
        "random_state": 42,
        "sampling_strategy": "all" if task_data.name == "MCI_CONTROL" else "not minority",
        "max_features": "sqrt",
        "class_weight": "balanced",
        "bootstrap": task_data.name == "AD_CONTROL",
        "n_jobs": 8,
    }
...
def train_xgboost(task_data: TaskData) -> xgb.XGBClassifier:
    ...
    if "ensembl_gene_id" in task_data.genes_df.columns:
        genes_with_ids = task_data.genes_df.copy()
    else:
        genes_with_ids = task_data.genes_df.reset_index(drop=False, inplace=False)
    plot_xgb_feature_importance(model, task_data.name, genes_with_ids, "brain")
    return model
...
def train_logistic_regression(task_data: TaskData) -> Pipeline:
    ...
    plot_lr_feature_importance(pipeline.named_steps["clf"], task_data.name, task_data.x_train, task_data.genes_df, "brain")
    return pipeline


def compare_models(task_data: TaskData, models: Dict[str, object], output_dir: Optional[Path], tissue: str) -> None:
    if not output_dir:
        output_dir = Path.cwd() / "brain_models"
...
def run_task(
    ...
    test_metadata: pd.DataFrame,
    genes_df: pd.DataFrame,
    config: BrainClassifierConfig,
) -> Dict[str, object]:
    if config.output_dir:
        os.environ["BRAIN_OUTPUT_DIR"] = str(config.output_dir)
    with task_logging(task_name):
        print(f"\n=== Running {task_name} Brain classification ===")
        ...
        describe_dataset(config.tissue_name, task_data.x_train, task_data.y_train, task_data.x_test, task_data.y_test, train_metadata, test_metadata)
        ...
        persist_model(rf_model, "brain_rf", task_name, task_data, config.output_dir or Path.cwd())
        persist_model(xgb_model, "brain_xgb", task_name, task_data, config.output_dir or Path.cwd())
        persist_model(lr_model, "brain_lr", task_name, task_data, config.output_dir or Path.cwd())
        return {"rf": rf_model, "xgb": xgb_model, "lr": lr_model}


def run_pipeline(config: BrainClassifierConfig, tasks: Optional[List[str]] = None) -> Dict[str, Dict[str, object]]:
    if config.output_dir:
    ...
    genes_ad, genes_mci = load_brain_gene_sets(config)

    results = {}
    selected_tasks = tasks or ["AD_CONTROL", "MCI_CONTROL"]
    if "AD_CONTROL" in selected_tasks:
        results["AD_CONTROL"] = run_task("AD_CONTROL", counts, train_metadata, test_metadata, genes_ad, config)
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Brain classification pipeline")
    ...
    parser.add_argument("--counts", type=Path, default=None, help="Path to residual counts CSV")
    parser.add_argument("--dlpfc-ad", type=Path, default=None, help="Path to DLPFC AD gene list")
    parser.add_argument("--dlpfc-mci", type=Path, default=None, help="Path to DLPFC MCI gene list")
    parser.add_argument("--pcc-ad", type=Path, default=None, help="Path to PCC AD gene list")
    parser.add_argument("--pcc-mci", type=Path, default=None, help="Path to PCC MCI gene list")
    parser.add_argument("--output-dir", type=Path, default=None, help="Directory for persisted models and plots")
...
def main() -> None:
    args = parse_args()
    # Set default output directory to files/brain
    default_output_dir = Path(__file__).resolve().parent / "files" / "brain"
    config = BrainClassifierConfig.default(args.output_dir or default_output_dir)
    if args.metadata:
    ...
    if args.dlpfc_ad:
        config.dlpfc_ad_genes_path = args.dlpfc_ad
    if args.dlpfc_mci:
        config.dlpfc_mci_genes_path = args.dlpfc_mci
    if args.pcc_ad:
        config.pcc_ad_genes_path = args.pcc_ad
    if args.pcc_mci:
        config.pcc_mci_genes_path = args.pcc_mci
    if args.output_dir:
        config.output_dir = args.output_dir
    else:
        config.output_dir = default_output_dir

    # Set environment variable for brain output
    os.environ["BRAIN_OUTPUT_DIR"] = str(config.output_dir)
